...
/// SCRT_THREADSAFE determines if atomic variables are used for thread safety of SimConnectRequestTracker methods.
#if !defined(SCRT_THREADSAFE)
	// No threading if being used in a MSFS WASM module.
	#if defined(_LIBCPP_HAS_NO_THREADS) || defined(_MSFS_WASM) || defined(__wasi__)
...
// SimConnect.h requires some MSVC types to be defined, which typically means including Windows.h.
// Supports building as part of a MSFS WASM module, which has its own Win types header.
...
public:
	...
	struct RequestData
	{
		...
		std::stringstream ssArguments;      ///< Parameter argument values passed in the invoker method. Stored in an I/O stream for potential retrieval as individual values.
